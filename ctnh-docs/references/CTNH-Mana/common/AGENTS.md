...
| 多方块基类 | `common/multiblock/ManaMultiBlockMachine.java`（`hatch`/`hatchPos`、`ManaLevel`、`isZenithOpen`、`Zenith_Enhanced`）、`BaseManaMultiBlockMachine.java`（魔力消耗、升级件、`ManaStatusGui`/`ShroudUi` 标签页）、`CrossParallelManaMultiBlockMachine.java`、`MultiPatternMultiblockMachine.java` |
