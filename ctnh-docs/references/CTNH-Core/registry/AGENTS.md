...
| 精准装配线 | `registry/machines/GTMachineModify.java#modifyGTAssembly()` —— `GCYMMachines.LARGE_ASSEMBLER` 的 supplier 为 `MultiblockComputationMachine::new`，并把 `CTNHRecipeTypes.PRECISION_ASSEMBLY_RECIPES` 追加进 recipeTypes |
...
- 注册类统一 `CTNH` 前缀。
- **中文名在注册处声明**（本域最重要的写法）：
  - 纯方块在 `CTNHBlocks` 注册时传入 `cnName`：`createCoilBlock(ICoilType, cnName)`、`createFireboxCasing(BoilerFireboxType, cnName)`、`createTurbineRotorBlock(name, R, G, B, A, cnName)`、`createRotateCasing(name, map, cnName)`（`(name, map)` 形参等价于 `cnName = null`，不声明中文名），内部统一 `.cnlang(cnName)`。
...
- `CTNHRecipeModifiers.ebfOverclock()` 的类型判断对象为 `RecipeElectricMultiblockMachine`；线圈机器匹配失败时经 `RecipeModifier.nullWrongType(CoilWorkableElectricMultiblockMachine.class, machine)` 报类型错误。需要判断机器类型的 `recipeModifier`（涡轮、发电机、`LaserSorter`、`SinopeChemical`、`CryotheumFreezer`、`PlasmaAlloyBlast`、`WideParticleAccelerator`、`SpacePhotovoltaicBaseStation`、`MeadowMachine` 等）在类型不匹配时经 `RecipeModifier.nullWrongType(<期望机器类>.class, machine)` 返回 `RecipeModifier.DEFAULT_FAILURE` 并记录开发期错误日志。
- **发电机配方修饰符**：`utils/CTNHMachineUtils.registerEfficiencyGeneratorMachines(...)` 链上为 `.recipeModifiers(GTRecipeModifiers.TIER_CHECK, recipeModifier)`，等级检查与传入修饰符同时生效；`HyperPlasmaTurbineRegister` 与 `MultiblocksA` 的化学燃料发电机用 `.recipeModifiers(true, GTRecipeModifiers.TIER_CHECK, <Machine>::recipeModifier)` 拦截超过机器等级的配方。效率发电机（硅岩反应堆、火箭引擎）共用 `EfficiencyGeneratorMachine.recipeModifier`，`CTNHRecipeModifiers.rocketEngine` 委托给它。
...
- 给已支持注册处声明中文名的方块/多方块再补 `@Key("block.ctnhcore.*")` + `Lang` 字段伪造翻译。
- 删除注册对象后留下悬空 lang 条目。
- 用 `WorkableElectricMultiblockMachine::new` 注册需要配方类型的多方块（不实现 `IRecipeLogicMachine`，模式页签不显示、配方类型不生效）；正确做法是用 `RecipeElectricMultiblockMachine` 或其子类。
